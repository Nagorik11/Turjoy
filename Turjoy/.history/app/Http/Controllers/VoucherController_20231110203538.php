...
class VoucherController extends Controller
{
    ...
    public function store(Request $request)
    {
        $request->validate([
            ...
            'origin' => 'required',
            'destiny' => 'required',
            'seat_quantity' => 'required|integer', // Adjust the validation rule as needed
            'base_rate' => 'required|numeric', // Adjust the validation rule as needed
        ]);
        ...
        $voucher->base_rate = $request->input('base_rate');
        $voucher->save();
    
        return redirect()->route('voucher.index');
    }
    
    In this example, we've added more specific validation rules for each input, and we're using the input method to fetch values from the request.
    
    Make sure to adapt the validation rules to match your actual validation requirements and adjust your view and routes accordingly.
