    ...
    <div class="card mx-auto mb-5 mt-4">
        ...
        <form style="margin: 20px auto 0; max-width:500px;">
            ...
            <div class="form-group">
                ...
                <div>
                    <select name="origin" class="selectpicker form-control" data-flag="true" data-width="500px" onchange="filterDestinations()">
            ...
            <div>
                <select name="destinationSelect" id="destinationSelect" class="selectpicker form-control" data-flag="true" data-width="500px">
                @foreach($routes as $route)
                @if($route->origin == 'user_selected_origin')
                    <option value="{{ $route->destiny }}">{{ $route->destination }}</option>
                @endif
            @endforeach
            </select>
            </div>
            ...
            <script>
                function filterDestinations() {
                    ...
                    const destinationSelect = document.querySelector('select[name="destiny"]');
                    const destinations = @json($routes->pluck('destiny'));

                    // Filter destinations based on the selected origin
                    const filteredDestinations = destinations.filter(destiny => destiny !== origin);

                    // Clear the current options in the destination select
                    destinationSelect.innerHTML = '';

                    // Add the filtered destinations as options in the destination select
                    ...
                    // Remove unmatching results
                    const options = destinationSelect.options;
                    for (let i = 0; i < options.length; i++) {
                        if (options[i].value === destiny) {
